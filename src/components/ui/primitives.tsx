...
export const StatusBadge = ({ status }: { status: string }) => (
  <span className={`badge border ${statusTone[status] ?? 'border-slate-200 bg-slate-100 text-slate-700'}`}>{status}</span>
);

export const Notice = ({ tone = 'info', children }: { tone?: 'info' | 'warning'; children: ReactNode }) => (
  <div className={`rounded-lg border p-3 text-sm ${tone === 'warning' ? 'border-amber-200 bg-amber-50 text-amber-900' : 'border-indigo-200 bg-indigo-50 text-indigo-900'}`}>
    {children}
  </div>
);
